...
//
// can assume lowercase letters, do not have to convert input to lowercase
// can assume user will enter only a single char (assumed to be a letter) , no other input validation required
// can read input as char or string

//the planning assignment now applies to this program
//one complete run-through og the program counts as a test
//need to create a project plan
// Professor does not like us to use "using namespace std"
